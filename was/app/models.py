...
from app.gitinfo import daemon_outdated as _daemon_outdated
...
class Device(Base):
    ...
    def daemon_outdated(self) -> bool:
        """배포된 저장소의 데몬 버전보다 낮은가 (규칙은 app/gitinfo.daemon_outdated).

        판정을 모델에 두는 이유 — DeviceResponse 를 돌려주는 곳이 목록·단건·상태변경·그룹배정
        네 군데라, 라우터마다 계산하면 한 곳만 고쳐져 화면끼리 어긋난다.
        """
        return _daemon_outdated(self.daemon_version)
