...
| 메뉴 | 기능 |
|---|---|
| 대시보드 | **사용자별 업로드/삭제 카운트**, **사용자별 업로드양·삭제량**(용량), 서비스 영향도, 최근 이상 알림. 카드는 손잡이를 잡고 끌어 순서를 바꿀 수 있고 순서는 브라우저에 저장된다. 2일을 넘는 기간은 **일 단위로 합산**해 그린다(선 그래프, x축은 MM/DD). 범례에서 계정을 클릭하면 그 계정만 표시된다 |
