...
대량 발생한다. 그래서 "CWD 실패 급증" 알림에는 그 버킷에서 실패가 가장 많은 경로를 함께 적고,
...
제외 경로 설정은 롤업(`service_metrics.cwd_fails`) · 알림 · 대시보드 실패 건수가 **같은 조건**
(`service_monitor.cwd_not_ignored_sql()`)을 쓴다. 대시보드 도넛은 제외 후 건수를 보여주고
범례·툴팁에 "제외 N건"을 함께 적는다.
