...
│       ├── login_config.py       # 로그인 화면 설정 (제목·경고문·배경·로고) 단일 소스
...
│           ├── settings.py       # 알림 설정 / 임계값 / 음소거 / DB 저장소 현황 / 로그인 화면
│           ├── telcos.py         # 통신사 관리
...
│               ├── settings-login.js # 로그인 화면 설정 + 로그인 창 렌더 (미리보기와 공용)
...
데몬 버전은 `ftp-daemon/soltrace_daemon.py` 의 `DAEMON_VERSION` 한 곳에만 적는다. WAS 는
**배포된 저장소의 같은 파일**에서 그 값을 읽어 '최신'으로 삼고, 장비가 하트비트로 보고한
값과 다르면 구버전으로 표시한다. 데몬이 자가 업데이트로 받아 가는 파일과 WAS 가 기준으로
삼는 파일이 같으므로 배포만 하면 기준이 저절로 맞는다.
자세한 내용은 [ftp-daemon/README.md](ftp-daemon/README.md#데몬-버전) 참고.
...
아이콘만 남는다. 관리자 전용 탭(장비 관리·그룹 관리·변경 이력·설정)은 고객 계정에서 숨겨지며 서버에서도 차단된다.
오른쪽 위에는 로그인한 계정(아이디 + 관리자/고객사)과 **다크 모드 토글**이 있다 — 테마는 브라우저에 기억된다.

| 메뉴 | 기능 |
|---|---|
| 대시보드 | **사용자별 업로드/삭제 카운트**, **사용자별 업로드양·삭제량**(용량), **장비 상태**, 전송/로그인 실패 건수, 최근 이상 알림. 카드는 손잡이를 잡고 끌어 순서를 바꿀 수 있고 순서는 브라우저에 저장된다. 2일을 넘는 기간은 **일 단위로 합산**해 그린다(선 그래프, x축은 MM/DD). 네 사용자별 카드는 오른쪽에 **사용자·최대·현재** 표가 붙고(정렬 가능), 계정을 클릭하면 그 계정만 표시된다 |
| 장비 상태 | 지금 진행 중인 이상 기준의 정상/주의/심각 도넛 + 장비 목록. 도넛(또는 범례)에서 상태를 클릭하면 그 상태의 장비만 남고, 장비마다 **진행 중인 지표**(전송 실패율 등)를 함께 보여준다 |
| 서비스 영향도 | 전송 실패율 / 로그인 실패율 / CWD 실패 도넛 차트, 알림 테이블, CWD 실패 원인 분석 |
| 장비 관리 | 신규 장비 확인(Confirm), 그룹 배정, 비활성화, 삭제, 원격 업데이트. 호스트명 아래에 **데몬 버전**이 보이고, 배포본과 다르면 ⚠︎ **구버전**으로 표시된다. 왼쪽 체크박스로 **여러 대를 골라 한꺼번에 업데이트** |
| 그룹 관리 | telco / service / other 유형으로 그룹 생성·수정·삭제, 그룹 행의 ↻ 로 **그 그룹 장비 일괄 업데이트**, 체크박스로 **여러 그룹을 골라 한꺼번에**. `장비` 버튼은 그 그룹 소속 장비만 보여주고(검색하면 미등록 장비까지) 데몬 버전을 함께 표시한다 |
| 로그 조회 | 텔코·고객사·그룹·사용자·IP·파일명·작업유형·기간 필터, **접근 계정 다중 선택**, 페이징(특정 페이지 직접 이동), CSV / XLSX 내보내기 (기본 90일). 탭으로 다시 들어오면 조건이 초기화된다 |
| 구성도 | 시스템 구성 · 이상 감지 알림 흐름 그림과 **인수인계 문서**(서버·경로, 자주 하는 작업, 장애 대응, 자동으로 도는 것, 인계 체크리스트). 비밀값은 적지 않고 **있는 위치**만 가리킨다 |
...
| 설정 | 알림 채널 (웹훅 / HMS), 이상 감지 임계값, 알림 음소거, **관리자 계정**, **로그인 설정**(로그인 화면 문구·배경·로고), 고객 계정(FTP 계정 매핑·비고·등록자), DB 저장소(보존 기간·월별 파티션 삭제·디스크 자동 정리) |
| 내 계정 (상단바) | 아이디를 클릭하면 **본인 비밀번호 변경** — 현재 비밀번호를 확인한다. 관리자·고객 계정 공용 |

### 로그인 설정 (로그인 화면)

설정 > 로그인 설정에서 로그인 창에 보이는 것을 바꾼다 — **제목**, **안내 문구**,
**접근 경고 메시지**(줄바꿈 유지, 최대 2000자), **배경 색상**(`#RRGGBB`),
**배경 이미지**(최대 2MB), **로고 이미지**(최대 512KB). 배경은 이미지가 색상보다 우선한다.

- **미리보기**: 저장 전 입력값 그대로 실제 로그인 창과 **같은 함수**(`applyLoginScreen`)로 그린다
  — 미리보기와 실제가 갈라지지 않는다. 이미지는 고르는 즉시 저장되고, 문구·색상은 저장을 눌러야 한다.
- **이미지 저장 위치**: 파일이 아니라 `app_config` 에 data URI 로 넣는다. 배포(`update_rocky8.sh`)가
  저장소를 통째로 갱신해도 남고, 백업이 DB 하나로 끝난다. 형식은 매직 바이트로 판정하므로
  (브라우저가 보낸 Content-Type 을 믿지 않는다) jpg · png · gif · webp 만 올라간다.
- **공개 조회**: `GET /api/v1/settings/login-config` 만 인증이 없다 — 로그인 **전에** 읽어야 하기
  때문이다. 그래서 이 설정에는 화면에 어차피 보이는 장식 값만 담는다(계정·IP·임계값은 넣지 않는다).

### 관리자 계정

관리자도 `users` 테이블에서 관리한다(설정 > 관리자 계정). `.env`/`app_config` 의 부트스트랩
관리자는 최초 기동 시 이 테이블로 자동 이관되며, 이관 전에도 로그인할 수 있도록 예전 경로가
남아 있다.

- **다계정**: 관리자를 여러 개 둘 수 있고, 계정을 추가한 뒤에는 기본 관리자도 비활성화할 수 있다.
- **잠금**: 비밀번호를 **5회 연속** 틀리면 계정이 **15분** 잠긴다. 실패 횟수와 잠금 시각은 DB에
  저장되므로 재시작·다중 워커에서도 같은 상태를 본다. 관리자가 목록에서 즉시 해제할 수 있고,
  비밀번호를 재설정해도 잠금이 풀린다.
- **안전장치**: 마지막 활성 관리자와 로그인 중인 본인 계정은 비활성화·삭제할 수 없다.
- **즉시 반영**: 계정을 비활성화하면 이미 발급된 토큰도 다음 요청에서 거부된다.
- **IP 제한**: 관리자에게는 전역 접속 허용 IP(설정 > 계정 보안)가 적용된다.

### 고객사 계정 (데이터 격리)

관리자(admin) 외에 고객사별 조회 전용 계정을 둘 수 있다. 설정 > 고객 계정에서 관리한다.

- **격리 기준 (두 겹)**: 계정마다 **FTP 계정 매핑**(`user_ftp_accounts`)을 등록한다 — 그룹 여러 개,
  그룹마다 FTP 아이디 여러 개.
  1. `device_scope` — 매핑에 등록된 **그룹의 장비**만 보인다(그 그룹이 계정의 `customer` 것인지도 함께 확인).
  2. `ftp_scope_sql` — 그 장비의 로그 중 **등록된 FTP 아이디의 것**만 보인다. 로그 조회·CSV/XLSX·
     총건수·대시보드 집계가 모두 같은 조건을 쓴다(`deps` 한 곳에 정의).
- **매핑이 없으면 아무것도 보이지 않는다.** 한 그룹의 장비를 여러 고객사가 함께 쓰는 경우가 있어,
  그룹만으로 열지 않고 "이 계정이 볼 FTP 아이디"를 명시해야 열리게 했다(2026-09-06 운영 결정).
  기존 계정도 매핑을 넣기 전까지는 빈 화면이 된다.
- **화면**: 고객 계정으로 로그인하면 왼쪽 탭에 **대시보드 · 로그 조회 · API 가이드**만 보인다.
- **비고 · 등록자**: 계정마다 비고(담당자·연락처·용도)를 적고, 목록에서 **누가 언제 만든 계정인지**
  (`created_by` · `created_at`) 확인한다.
...
- **service_metrics 기반 카드**(장비 상태·추이)는 FTP 아이디 단위 값이 아니라 장비 단위 집계라
  장비 범위까지만 제한된다. 계정별 값이 필요한 화면은 모두 `ftp_logs` 기반이다.
...
- **원인이 밝혀진 계정의 전송 실패는 뺄 수 있다**: 병렬 업로드가 몰려 ftp·s3fs 지연으로 일부가
  실패하고 고객사가 재처리로 마무리하는 계정처럼, 장비 상태와 무관한 실패가 실패율을 늘 띄워
  진짜 장애를 묻는 경우가 있다. **설정 > 알림 설정 > 전송 실패 제외 계정**에 계정을 넣으면
  롤업·알림·대시보드 실패 건수에서 그 계정의 **실패만** 빠진다(성공 건과 전송량은 그대로 세고,
  로그 조회에도 그대로 남는다). 대시보드 도넛 범례에 `(제외 N건)` 으로 숨긴 건수를 함께 밝힌다.
  조건은 `service_monitor.xfer_not_ignored_sql()` 한 곳에 있어 롤업·화면 숫자가 갈라지지 않는다.
- **클라이언트가 잘못 보낸 요청은 지표에서 뺀다**: 파일명 없는 전송(`STOR /vod//`)이나 명령
  문법 오류처럼 서버 상태와 무관하게 늘 실패하는 요청은 `client_error` 로 따로 기록한다.
  로그 조회에서는 작업 유형 **클라이언트 오류**로 보이지만 전송 실패율·알림에는 들어가지 않는다
  (`cwd_fail` 과 같은 취지 — 장비를 의심하며 쫓게 되는 것을 막는다).
  데몬 1.1.6 이전에 쌓인 과거분은 `upload`/`download` + `fail` 로 남아 실패율을 부풀리므로,
  [`scripts/reclassify_client_error.sql`](scripts/reclassify_client_error.sql) 로 같은 기준에
  맞춰 옮긴다(행을 지우지 않고 세 컬럼만 바꾸며, 1만 행마다 커밋해 재실행 안전).
...
- **장비 상태는 '지금' 기준**: 대시보드의 장비 상태 도넛은 선택한 기간이 아니라 **진행 중인 에피소드**
  (`service_alert_episodes.resolved_at IS NULL`)로 판정한다. 기간 내 알림을 세면 새벽에 잠깐 났다
  회복된 장비가 그 기간 내내 주의로 남아, 화면을 보는 시점의 상태를 알 수 없었다. 같은 응답의
  알림 목록·추이·실패 건수는 종전대로 선택한 기간이다
...
| 시점 | 발송 |
|---|---|
| 이상 시작 (직전 버킷이 정상) | 보내지 않음 — 이어지는지 본다 (아래 "짧은 흔들림") |
| 이상이 `ALERT_MIN_STREAK_BUCKETS`(기본 2, 20분)만큼 이어짐 | **보냄** (에피소드당 한 번) |
| 그 뒤로도 이어지는 버킷들 | 보내지 않음 — 화면(`service_alerts`)에는 그대로 쌓임 |
| 등급이 **심각** | **보냄** (지속 버킷 수와 무관하게 즉시) |
| 등급 상승 (주의 → 심각) | **보냄** (한 번) |
| 마지막 이상 버킷 이후 30분(3버킷) 조용 | 에피소드 종료 → **복구 알림** |
| 복구 뒤 다시 나빠짐 | 새 에피소드로 **다시 보냄** |

**짧은 흔들림은 아예 보내지 않는다.** 10분 한 버킷만 임계를 넘었다가 스스로 돌아오는 이상은
사람이 손쓸 새도 없이 끝나는데 메시지는 *감지 + 복구* 두 건이 나간다. 발송 전 지속 조건
(`ALERT_MIN_STREAK_BUCKETS`)을 넘기지 못한 에피소드는 이상 알림이 나간 적이 없으므로
복구 알림도 나가지 않고, 대시보드와 `service_alerts` 에만 남는다. 하나하나 다 받고 싶으면
설정에서 **1** 로 두면 종전처럼 즉시 발송한다.
...
| 발송 전 이상 지속 버킷 수 (`ALERT_MIN_STREAK_BUCKETS`) | 2 (20분) | 이만큼 이어질 때만 발송 · **1 = 즉시**, 심각은 항상 즉시 |
...
| 전송 실패 제외 계정 | (없음) | 설정 페이지 전용(여러 줄) · 한 줄에 하나, `*` 와일드카드 · 성공 건은 그대로 집계 |
...
그 경로가 실제로 있었다는 증거가 있으면 **집계·추이·알림·분석 목록은 물론 로그 조회 목록에서도
뺀다** — 실패가 아닌 것을 실패로 보여줄 이유가 없다. 증거는 두 가지다.

| 증거 | 창 | 뜻 |
|---|---|---|
| 같은 경로(또는 하위)가 `mkdir` 로 생성 | −5초 ~ +10분 | CWD → 550 → MKD → 업로드 흐름 |
| 같은 경로 아래로 `upload` **성공** | ±5초 | 만들 것도 없이 이미 있었다 |

'직후'가 5초 앞까지인 것은(`_CWD_CLOCK_SKEW`) 같은 흐름인데도 뒤 명령이 CWD 550 보다 1~2초
이른 시각으로 기록되는 일이 있어서다 — 엄격히 '이후'만 보면 정상 흐름이 실패로 남는다.

두 증거를 **각각 EXISTS 로 두고 OR** 하는 이유는 인덱스다. 하나로 합치면 mkdir 부분 인덱스
(`idx_ftp_logs_mkdir_path`)도, 업로드 쪽 `idx_ftp_logs_device_time` 도 못 타고 파티션을 훑는다.
업로드 창이 ±5초로 좁은 것도 같은 이유다 — 업로드는 로그의 대부분이라 창을 10분으로 열면
실패가 몰리는 구간(=알림이 필요한 순간)에 조회가 무거워진다. 다만 xferlog 는 전송 **완료** 시각을
남기므로 **오래 걸린 큰 파일 업로드는 이 창을 벗어나** 증거로 잡히지 않는다.
(업로드 경로는 xferlog, CWD 경로는 확장로그에서 오므로 두 경로 표기가 어긋나는 환경에서는
업로드 증거가 매칭되지 않을 수 있다 — 그 경우 mkdir 증거만 동작한다.)
...
반면 **설정의 '제외 경로'는 로그 조회에 그대로 남는다** — 그건 실패가 아니라는 판정이 아니라
"원인을 알고 있으니 집계에서만 빼자"는 운영자의 선택이기 때문이다. 그래서 "CWD 실패 급증" 알림에는 그 버킷에서 실패가 가장 많은 경로를 함께 적고,
...
- 목록에는 **진짜 이동 실패만** 올라온다(로그 조회 화면도 마찬가지). 요약 줄에 "원본 N건 중 존재 확인 M건, 제외 경로 K건 제외"로 근거를 밝힌다.
...
| `ExtendedAllLog` | 로그인 성공(PASS 230), 로그인 실패(PASS 530 — 계정은 같은 세션의 `USER` 명령에서 가져온다), 로그아웃(QUIT), 이름변경(RNTO), 폴더생성(MKD 257), 폴더삭제(RMD 250), 디렉토리 이동 실패(CWD 550) |
...
| `GET` | `/api/v1/logs/usernames` | 같은 필터에서 접근한 FTP 계정 목록 (계정 다중 선택용) |
...
| `POST` | `/api/v1/users/{id}/unlock` | 실패 초과로 잠긴 계정 해제 |
| `GET` | `/api/v1/settings/changelog` | 배포된 `changelog.md` 를 날짜별로 파싱 (변경 이력 화면) |
| `POST` | `/api/v1/auth/password` | 본인 비밀번호 변경 (현재 비밀번호 확인) |
...
`ftp_logs`는 `log_time` 기준 월별 파티션으로 분할된다. WAS 기동 시(및 `init.sql` 적용 시) **과거 보존기간 ~ 향후 2개월** 파티션을 자동 생성한다. 보존 기간 안의 과거 로그를 bulk import 해도 default 파티션으로 빠지지 않게 하기 위해서다.
